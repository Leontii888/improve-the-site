...
(async function getRateUsd(){
	...
	console.log(`Смотрите также исторические данные getRateUsd`)
		...
		curdollarUsa = (RUB*1.015).toFixed(1);
...
const fooProfit = (deposit, kf, sample, success) => {
	let currd = 0,
	arrayCount = [],
...
let {
	...
	kf:kf,
	sample:sample,
	bestProcent:bestProcent} = fooProfit(100000, 2, 30, 40),
...
const getDataMinn= ()=> {
	...
	.map(// .replace(/\s/g, '')
	c=> parseInt(c));
	defaultData = defaultData.length!=4
	?[100,2,10,30]
	:defaultData
...
const countMinn = ()=> {
	...
	let {
		...
		// dep = dep!='undefined'?dep:100;
		// success = success!='undefined'?success:50;
		// sample = sample!='undefined'?sample:10;
		// kf = kf!='undefined'?kf:2;
