...
hosting(library);
 ...
 // to LS
// distributeItemsToLocalStorage(titles,library)
function hosti9ng(arr){
	titles.forEach((titleEl,i)=> {
				let titleItem = arr.filter((el,i)=> Object.keys(el) == titleEl);
				titleToObj = titleItem[0];
				prepareList(titleToObj[Object.keys(titleToObj)[0]]);
...
// console.log(JSON.parse(localStorage.getItem("css"))[0]
// 	.filter(el => el.address.matches('cssgrid'))
// 	)

// hostingFromLS(JSON.parse(localStorage.getItem("css"))[0])


let myLSArr = {};

for (let item in localStorage){
	myLSArr ={...myLSArr,item: JSON.parse(localStorage.getItem(item))}
	// myLSArr= [...myLSArr,]
}
